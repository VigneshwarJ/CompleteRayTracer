...
namespace Utils {
		static uint32_t GetVulkanMemoryType(VkMemoryPropertyFlags properties, uint32_t type_bits)
		{
			...
			vkGetPhysicalDeviceMemoryProperties(MainApp::GetPhysicalDevice(), &prop);
	...
	{
		VkDevice device = MainApp::GetDevice();
	...
	void Image::Release()
	{
		MainApp::SubmitResourceFree([sampler = m_Sampler, imageView = m_ImageView, image = m_Image,
	...
	void Image::SetData(const void* data)
	{
		...
		// Copy to Image
		{
			VkCommandBuffer command_buffer = MainApp::GetCommandBuffer(true);
			...
			MainApp::FlushCommandBuffer(command_buffer);
		}
	}
